...
export class PropertyService {
	...
	public async getProperty(memberId: ObjectId, propertyId: ObjectId): Promise<Property> {
		...
		targetProperty.memberData = await this.memberService.getMember(null as unknown as ObjectId, targetProperty.memberId);
		return targetProperty;
	}

	public async updateProperty(memberId: ObjectId, input: PropertyUpdate): Promise<Property> {
		const { _id, soldAt, deletedAt } = input;
		const search: T = {
			_id: _id,
			memberId: memberId,
			propertyStatus: PropertyStatus.ACTIVE,
		...
		if (input.propertyStatus === PropertyStatus.SOLD) input.soldAt = moment().toDate();
		else if (input.propertyStatus === PropertyStatus.DELETE) input.deletedAt = moment().toDate();
